...
## Modernizing Oracle Infrastructure

Oracle databases don't have to live on legacy infrastructure. This project runs **Oracle 26ai** — the latest Oracle Database release with built-in AI Vector Search and JSON Relational Duality — on a fully modern, cloud-native stack:

- **Kubernetes** as the control plane for all infrastructure lifecycle management
- **KubeVirt** to run Oracle as virtual machines inside Kubernetes, combining VM isolation with Kubernetes orchestration
- **Red Hat Enterprise Linux 9** as the guest OS — the certified, hardened platform for Oracle Database with SELinux, cgroups v2, and long-term support
- **Ansible** as the declarative automation engine — every phase from VM provisioning to Data Guard configuration is idempotent and repeatable
- **Terraform** to provision the cloud observability layer as code — datastreams, datasets, OPAL transforms, and dashboards
- **OpenTelemetry** as the vendor-neutral telemetry standard, shipping metrics and logs to any OTLP-compatible backend

The result: Oracle RAC with Data Guard and FSFO, running on RHEL 9 VMs inside Kubernetes, fully provisioned by two Ansible commands, with production-grade observability wired in from the start. No manual steps. No ClickOps. Everything in Git.
...
The following diagram shows **Topology C** — the most complete variant — with every IP-addressable component and its role in the cluster:

```mermaid
flowchart LR
    Client(["fa:fa-laptop <b>App Client</b>"])
    OTEL(["fa:fa-chart-line <b>OTEL Collector</b>"])

    Client -- "JDBC / OCI" --> SCAN

    subgraph NET["L2 VXLAN &ensp;192.168.10.0/24 &ensp; br-vm .1 &ensp;·&ensp; vxlan100 VNI 100"]
        direction TB

        SCAN(["<b>SCAN VIP</b><br/>.100 :1521"])

        SCAN --- N1 & N2

        N1["<b>rac-node1</b> &ensp; .11 &ensp; VIP .21<br/>Grid · +ASM1 · RACDB1 · :1521<br/>4 vCPU · 8 Gi"]
        N2["<b>rac-node2</b> &ensp; .12 &ensp; VIP .22<br/>Grid · +ASM2 · RACDB2 · :1521<br/>4 vCPU · 8 Gi"]

        N1 <-- "Cluster Interconnect" --> N2

        N1 & N2 -. "Redo Transport" .-> SB

        SB["<b>rac-standby</b> &ensp; .13<br/>Grid · +ASM_STBY · Standby DB<br/>Real-time Redo Apply · :1521"]

        OBS["<b>rac-observer</b> &ensp; .14<br/>FSFO Observer · Instant Client<br/>Auto-failover trigger"]

        OBS -. "DGMGRL" .-> N1 & SB

        N1 & N2 --- DISK[("fa:fa-database <b>Shared ASM</b><br/>+DATA 20G · +FRA 10G · +VOTE 2G")]
    end

    OTEL -- "oracledb :1521" --> N1 & N2 & SB
    OTEL -- "prometheus :9100" --> N1 & N2 & SB
    OTEL -. "filelog" .-> N1 & N2 & SB

    style NET fill:#f8f9fa,stroke:#1a508b,color:#1a1a2e
    ...
    style DISK fill:#e94560,stroke:#c0392b,color:#fff
...
flowchart TB
    ...
    style K8s fill:#ffffff,stroke:#d0d7de,color:#1a1a2e
    style VXLAN fill:#e8f0fe,stroke:#1a508b,color:#1a1a2e
    style Multus fill:#f0f4f8,stroke:#1a508b,color:#1a1a2e
    style ASM fill:#fef2f2,stroke:#e94560,color:#1a1a2e
...
```mermaid
flowchart LR
    ANS(["fa:fa-cogs <b>Ansible</b><br/>otel-controller.yaml"])

    subgraph SOURCES["Oracle RAC VMs"]
        direction TB
        NE["node_exporter<br/>+ cron collector.sh"]
        DB["Oracle DB<br/>:1521"]
        LOGS["Log files<br/>/u01/app/..."]
    end

    subgraph WIRE["Stage 3 — RAC Receivers"]
        direction TB
        PR["prometheus receiver<br/>:9100 + textfile"]
        OR["oracledb receiver<br/>SQL :1521 per node"]
        FL["filelog receiver<br/>alert · listener · audit"]
    end

    subgraph DEPLOY["Stage 2 — OTEL Collector · K8s"]
        direction TB
        CM["ConfigMap<br/>merged YAML"]
        COL["<b>Collector Pod</b><br/>K8s Deployment"]
        SVC["Service<br/>:4317 · :4318"]
        CM --> COL
        SVC --> COL
    end

    subgraph CLOUD["Cloud Observability Platform — e.g. Observe Inc"]
        direction TB
        DS["<b>Datastream</b><br/>oracle-rac-otel<br/>OTLP/HTTP ingest"]
        DS --> DSET["12 Datasets<br/>OPAL extraction · fan-out"]
        DSET --> DASH["3 Dashboards<br/>Overview · SQL · Waits&IO<br/>30+ widgets"]
    end

    TF["<b>terraform apply</b><br/>observe-terraform/"]

    ANS -- "Stage 1: runs" --> TF
    ANS -- "Stage 2: deploys" --> COL
    ANS -- "Stage 3: wires receivers<br/>+ installs node_exporter" --> WIRE
    ANS -- "installs collector.sh<br/>+ cron" --> SOURCES

    TF -- "creates datastream,<br/>datasets, dashboards" --> CLOUD
    TF -. "returns ingest token<br/>+ endpoint URL" .-> ANS
    ...
    COL -- "OTLP/HTTP<br/>bearer token" --> DS

    style ANS fill:#3498db,stroke:#2471a3,color:#fff
    style SOURCES fill:#fef2f2,stroke:#e94560,color:#1a1a2e
    style WIRE fill:#fff8f0,stroke:#e67e22,color:#1a1a2e
    style DEPLOY fill:#f0faf4,stroke:#27ae60,color:#1a1a2e
    style CLOUD fill:#f3e8ff,stroke:#8e44ad,color:#1a1a2e
    style TF fill:#4a90d9,stroke:#2c5f9e,color:#fff
    style DS fill:#8e44ad,stroke:#6c3483,color:#fff
    style DSET fill:#9b59b6,stroke:#7d3c98,color:#fff
    style DASH fill:#8e44ad,stroke:#6c3483,color:#fff
    style COL fill:#27ae60,stroke:#1e8449,color:#fff
    style CM fill:#2ecc71,stroke:#27ae60,color:#fff
    style SVC fill:#2ecc71,stroke:#27ae60,color:#fff
    style PR fill:#e67e22,stroke:#c0641a,color:#fff
    style OR fill:#e67e22,stroke:#c0641a,color:#fff
    style FL fill:#e67e22,stroke:#c0641a,color:#fff
    style NE fill:#e94560,stroke:#c0392b,color:#fff
    style DB fill:#e94560,stroke:#c0392b,color:#fff
    style LOGS fill:#e94560,stroke:#c0392b,color:#fff
```
